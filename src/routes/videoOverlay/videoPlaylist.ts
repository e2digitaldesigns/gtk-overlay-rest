...
import { VideoPlaylistModel, VideoPlaylistItemModel } from "../../models/vro_playlist.model";
...
router.post("/addItemToDefault", async (req: Request, res: Response) => {
  const { userId, username, videoId, videoThumbnail, videoTitle, videoUrl } = req.body;
...
// Load Playlist
router.get("/loadPlaylist/:userId/:playlistId", async (req: Request, res: Response) => {
  try {
...
router.get("/getUserPlaylists", verifyToken, async (req: Request, res: Response) => {
  try {
...
router.post("/addVideoToPlaylist", verifyToken, async (req: Request, res: Response) => {
  const twitchAuth = await TwitchAuthModel.findOne({
  ...
  try {
    const newItem = await VideoPlaylistItemModel.findOneAndUpdate(
      ...
      {
        $set: {
          playlistId: req.body.playlistId ? new ObjectId(req.body.playlistId) : null,
...
router.post("/removeVideoFromPlaylist", verifyToken, async (req: Request, res: Response) => {
  try {
...
router.post("/getPlaylistItems", verifyToken, async (req: Request, res: Response) => {
  try {
    const playlistItems = await VideoPlaylistItemModel.find({
      userId: new ObjectId(res.locals.userId),
      playlistId: req.body.playlistId === "default" ? null : new ObjectId(req.body.playlistId)
    })
      .select({ __v: 0 })
      .exec();
...
router.post("/editPlaylist", verifyToken, async (req: Request, res: Response) => {
  try {
...
router.get("/createPlaylist", verifyToken, async (req: Request, res: Response) => {
  try {
...
router.post("/deletePlaylist", verifyToken, async (req: Request, res: Response) => {
  try {
...
router.post("/updatePlaylistItem", verifyToken, async (req: Request, res: Response) => {
  try {
...
router.post("/videoRequest", async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (videoData.success && videoData.urls.sd) {
      const action = req.body.command === "!pvr" ? "priority-video-request" : "video-request";
